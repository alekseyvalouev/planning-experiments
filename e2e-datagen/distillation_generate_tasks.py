...
class TaskBuilder:
    ...
    def generate_task(self, single_step=False):
        if single_step:
            path = self._make_single_step_task()
        else:
    ...
    def _generate_path(self):
        ...
        use_full_path = random.choice([True, False])
        if use_full_path:
            print("Using full path.")
            full_path = path_1[:-1] + path_2
        else:
            print("Using partial path.")
            full_path = random.choice([path_1, path_2])
    ...
    def _make_single_step_task(self):
        start_node = random.choice(self.graph.nodes)
        next_node = random.choice(start_node.connections)
...
if __name__ == "__main__":
    #random.seed(40)
    builder = TaskBuilder("graph_vint_loose.json")
    path, task = builder.generate_task(single_step=False)
    if path:
