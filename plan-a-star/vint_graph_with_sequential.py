...
class Graph:
    def __init__(self, scenes, annotation_folder, sparsification_steps=4, drop_modality_p=0.0, dummy=False, name="graph.json"):
        ...
        self.fw_connectivity_limit = 3
        self.bw_connectivity_limit = 0
        self._load_vint()
    ...
    def _build_graph(self):
        ...
        while pos < len(pairs):
            ...
            while pos < len(pairs) and len(batch_pairs) < BATCH_SIZE:
                ...
                # check if they are sequentially connected
                node_scene = node.subnodes["V"]["image"].split("/")[-2]
                other_scene = other_node.subnodes["V"]["image"].split("/")[-2]
                if node_scene == other_scene:
                    node_img_idx = int(node.subnodes["V"]["image"].split("/")[-1].split(".")[0])
                    other_img_idx = int(other_node.subnodes["V"]["image"].split("/")[-1].split(".")[0])
                    if other_img_idx > node_img_idx:
                        if other_img_idx - node_img_idx <= self.fw_connectivity_limit*self.sparsification_steps:
                            connected.add((node.node_id, other_node.node_id))
                            node.add_connection(other_node)
                            continue
                    else:
                        if node_img_idx - other_img_idx <= self.bw_connectivity_limit*self.sparsification_steps:
                            connected.add((node.node_id, other_node.node_id))
                            node.add_connection(other_node)
                            continue
